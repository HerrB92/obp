...
/**
 * Test class for testing tag data encoded using protocol 26 (RFBPROTO_BEACONTRACKER_EXT).
 * 
 ...
 * @version 1.0
 **/
public class TestTagSightingProtocol26 {
	...
	public final void testTagSighting() {
		...
		payload[0] = (byte)Constants.RFBPROTO_BEACONTRACKER_EXT;	// Tag protocol
		payload[1] = (byte)0x04;							// Tag id (1)
		payload[2] = (byte)0x73;							// Tag id (2)
		payload[3] = (byte)Constants.RFBFLAGS_SENSOR;		// Flags, button pressed
		payload[4] = (byte)0x02;							// Strength: 2
		payload[5] = (byte)0x04;							// Last Seen Reader? (1)
		payload[6] = (byte)0xf4;							// Last Seen Reader? (2)
		payload[7] = (byte)0x2e;							// Time (1)
		payload[8] = (byte)0x97;							// Time (2)
		payload[9] = (byte)0x03;							// Battery
		payload[10] = 0;									// Sequence (1), some value
		payload[11] = 0;									// Sequence (2)
		payload[12] = (byte)0x01;							// Sequence (3)
		payload[13] = (byte)0xb0;							// Sequence (4)
		...
		assertEquals(26, tagSighting.getTagProtocol());
		assertEquals(2, tagSighting.getFlags());
		assertEquals(2, tagSighting.getStrength());
		assertNull(tagSighting.getProximitySightings());
		assertEquals(11927, tagSighting.getTagTime());
		assertEquals(3, tagSighting.getTagBattery());
		assertEquals(432, tagSighting.getTagSequence());
		...
		assertEquals("111,192,1,1,4,244,0,32,0,0,1,176,0,17,46,151,69,240,111,130,249,127,71,107,62,128,205,127,158,184,27,39",
